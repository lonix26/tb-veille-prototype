...
import { useDonnees, nb, pct, clsVar, nomZone, dateCH, MarkdownLeger, LienSource,
         alertesSignificatives, SEUIL_POIDS_PCT,
         BadgeCommentaire, signatureCommentaire,
  ...
  return (
    <div className="carte commentaire">
      ...
      <div className="c-pied">
        <BadgeCommentaire c={c} />
        {signatureCommentaire(c, dateCH)}
...
export default function CetteSemaine() {
  ...
  return (
    <div className="page">
      ...
      <h2 className="s-titre">Mouvements inhabituels</h2>
      {alertes.length === 0 ? (
        <div className="vide">Aucun mouvement inhabituel diffusable.</div>
      ) : (
        <>
          ...
          <div className="liste-signaux">
            {majeures.map((a, i) => (
              <div key={i} className="ligne-signal">
                ...
                <span className={"etq " + SENS_ETQ[sensMouvement(D, a)][0]}>{SENS_ETQ[sensMouvement(D, a)][1]}</span>
                <span className="prov">
                  vu &lt; 1 fois sur 10 sur cette série
