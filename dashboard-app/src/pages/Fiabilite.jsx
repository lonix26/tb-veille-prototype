...
function reservesDuDispositif(D, S) {
  ...
  // 6. La file d'examen, et la règle qui la borne.
  //
  // Depuis le 26.08, la file n'est plus la réserve : la RÈGLE l'est. Le
  // filtrage à seuil a déplacé le contrôle humain de l'item vers l'énoncé,
  // et un énoncé non audité est une décision sans preuve. Tant que
  // l'échantillon n'a pas été relu, le taux de faux négatifs est INCONNU —
  // et l'écran doit l'écrire, pas le taire.
  const file = Number(S?.items_en_attente_examen) || 0;
  const flt = S?.filtrage;
  ...
  if (flt && Number(flt.faux_negatifs) > 0) r.push({
    gravite: "grave", ou: "Exploitation",
    titre: "L'audit a trouvé des items mal écartés",
    corps: `${nb(flt.faux_negatifs)} faux négatifs sur ${nb(flt.items_audites)} items audités. La règle de seuil laisse passer des items pertinents : elle doit être relevée ou son garde-fou élargi. Les items concernés sont revenus dans la file d'examen.`
  });
  if (file > 300) r.push({
...
// `vueInitiale` n'est pas une option d'interface : elle permet à la vérification
// de rendre CHAQUE onglet. Un panneau derrière un onglet non visité est un
// panneau non testé — c'est ainsi que la page s'était cassée le 25.08.
export default function Fiabilite({ vueInitiale }) {
  const { D, S } = useDonnees();
  const [vue, setVue] = useState(vueInitiale || "reserves");
  ...
  return (
    <div className="page">
      ...
      {vue === "filtrage" && (() => {
        const flt = S?.filtrage, regle = S?.filtrage_regle, ech = S?.filtrage_echantillon || [];
        if (!flt) return <div className="vide">Le bilan de filtrage ne répond pas.</div>;
        return (
          <>
            <p className="bloc-intro">
            ...
            <p className="bloc-intro">
              La règle ci-dessous écarte ces items sans examen. <strong>Le contrôle humain ne
            ...
            <div className="tuiles" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", marginBottom: 18 }}>
              <div className="carte kpi"><div className="k-l">Items collectés</div>
                <div className="k-v">{nb(flt.items_collectes)}</div>
                <div className="k-s">depuis l'ouverture de l'étage 2</div></div>
              <div className="carte kpi"><div className="k-l">Écartés par règle</div>
                <div className="k-v">{nb(flt.items_filtres)}</div>
                <div className="k-s">{nb(flt.part_filtree_pct)} % du triable</div></div>
              <div className="carte kpi"><div className="k-l">File humaine</div>
                <div className="k-v">{nb(flt.file_humaine)}</div>
                <div className="k-s">{nb(flt.examens_humains)} examen(s) déjà rendu(s)</div></div>
              <div className="carte kpi"><div className="k-l">Faux négatifs</div>
                <div className="k-v">{flt.items_audites > 0 ? nb(flt.taux_faux_negatifs_pct) + " %" : "inconnu"}</div>
                <div className="k-s">{nb(flt.items_audites)} item(s) audité(s)</div></div>
            </div>
            ...
            <div className="carte" style={{ marginBottom: 16 }}>
              <h3 className="bloc-titre">L'état de l'audit</h3>
              <p className={"avert" + (flt.items_audites === 0 || flt.faux_negatifs > 0 ? " avert-fort" : "")}
            ...
            {ech.length === 0
              ? <div className="vide">Échantillon du mois entièrement audité.</div>
              : (
                <div style={{ overflowX: "auto" }}>
                  <table className="tableau">
                    <thead><tr>
                      <th>Rang</th><th>Note</th><th>Famille</th><th>Titre</th><th>Publié</th>
                    </tr></thead>
                    <tbody>
                      {ech.map(e => (
                        <tr key={e.item_id}>
