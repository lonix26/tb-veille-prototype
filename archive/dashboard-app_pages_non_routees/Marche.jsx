...
import { useDonnees, nb, pct, dateCH, LienSource, MarkdownLeger, estAgregat, nomZone,
         relu, BadgeCommentaire, TYPE_EVT, SENS_EVT, BadgeEvenement } from "../api.jsx";
      ...
      {commentaire && (
        <>
          <h2 className="section">
            {relu(commentaire) ? "La lecture validée" : "La lecture du moment"}
          </h2>
          <div className="commentaire">
            <div className="com-tete">
              <BadgeCommentaire c={commentaire} />
              <span>{relu(commentaire) ? commentaire.validated_by : "aucune relecture humaine"}</span>
              <span className="com-date">
                collecte n° {commentaire.run_id ?? "—"} ·{" "}
                {dateCH(relu(commentaire) ? commentaire.validated_at : commentaire.created_at)}
              </span>
            </div>
